...
export function generateContactEmail(data: ContactEmailData): string {
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
      <tr>
        <td style="padding-bottom: 12px; border-bottom: 2px solid #2563eb;">
          <h3 style="margin: 0; font-size: 15px; font-weight: 700; color: #2563eb; text-transform: uppercase; letter-spacing: 0.5px;">Contact Information</h3>
        </td>
      </tr>
  ...
  const messageSection = `
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
      <tr>
        <td style="padding-bottom: 12px; border-bottom: 2px solid #2563eb;">
          <h3 style="margin: 0; font-size: 15px; font-weight: 700; color: #2563eb; text-transform: uppercase; letter-spacing: 0.5px;">Message</h3>
        </td>
      </tr>
      <tr>
        <td style="padding: 8px 0;">
          <p style="margin: 0; color: #475569; font-size: 14px; line-height: 1.6; background: #f8fafc; padding: 12px 16px; border-radius: 6px; border-left: 3px solid #2563eb;">${sanitize(data.message)}</p>
        </td>
      </tr>
    </table>
  `
...
<head>
  ...
  <title>New Contact Message - ${sanitize(data.siteName)}</title>
</head>
  ...
  <table style="width: 100%; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-collapse: collapse;">
    <tr>
      <td style="background: linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%); padding: 32px 40px; text-align: center; border-radius: 0;">
        <div style="margin-bottom: 8px;">
          <span style="font-size: 36px; font-weight: 800; color: #ffffff; letter-spacing: 1px;">${sanitize(data.siteName)}</span>
        </div>
    ...
    <tr>
      <td style="padding: 32px 40px 8px;">
        <h2 style="margin: 0 0 4px; font-size: 22px; font-weight: 700; color: #1e293b;">New Contact Message</h2>
        <p style="margin: 0 0 20px; color: #64748b; font-size: 13px;">A customer has submitted a new message through the website.</p>
